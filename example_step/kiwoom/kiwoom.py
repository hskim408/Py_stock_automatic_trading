...
class Kiwoom(QAxWidget) :
    ...
    def get_account_info(self):
        ...
        self.account_num = re.sub(r'[^0-9]', '', account)
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "계좌평가잔고내역요청":
            ...
            print("종목 상세 정보 %s" % self.account_stock_dict)
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간" :
            ...
            self.portfolio_stock_dict[sCode].update({"등략율": d})
           ...
           # 등락율이 2.0% 이상이고 오늘 산 잔고에 없을 경우
            ...
            for order_num in not_meme_list:
                ...
                if order_gubun == "매수" and not_quantity > 0 and e > meme_price:
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["매수취소", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.account_num, 3, sCode,
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFidList):
        ...
        if int(sGubun) == 0:
            ...
            order_number = self.dynamicCall("GetChejanData(int)",
                                            self.realType.REALTYPE['주문체결']['주문번호'])

            order_status = self.dynamicCall("GetChejanData(int)",
                                            self.realType.REALTYPE['주문체결']['주문상태'])
            ...
            not_chegual_quan = self.dynamicCall("GetChejanData(int)",
                                                self.realType.REALTYPE['주문체결']['미체결수량'])
            ...
            chegual_time_str = self.dynamicCall("GetChejanData(int)",
                                                self.realType.REALTYPE['주문체결']['주문/체결시간'])
            chegual_price = self.dynamicCall("GetChejanData(int)",
                                             self.realType.REALTYPE['주문체결']['체결가'])
    ...
    #송수신 메세지 get
    def msg_slot(self, sCrcNo, sRQName, sTrCode, msg):
        print("스크린: %s, 요청이름: %s, tr코드: %s --- %s" %(sCrcNo, sRQName, sTrCode, msg))
